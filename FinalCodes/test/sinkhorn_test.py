...
loss_dict_notdot = {'random': 2e-3, 'ellip': 2., 'Caffa': 3.9}

f = open("test/sinkhorn_test.txt", 'w+')
...
for data in ['DOTmark', 'random', 'Caffa', 'ellip']:
    if data == 'DOTmark':
        ...
        for imageclass in imageclass_list:
            ...
            print(data, imageclass, 'sinkhorn', file=f)
            start = time.time()
            s = os.popen("python sinkhorn.py " + "--data " + data
        ...
        for n in [16, sqrt(512), 32, sqrt(2048)]:
            ...
            print(data, int(n ** 2), 'sinkhorn', file=f)
            start = time.time()
            s = os.popen("python sinkhorn.py" + " --data " + data
                         + " --n " + str(n) + " --eps " +
                         str(loss_dict_notdot[data] / log(n ** 4))).read()
            print('once time=', time.time() - start)
            print(s, file=f)
